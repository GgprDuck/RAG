...
class Reranker {
    ...
    async listwiseLlmRerank(query, results, topK) {
        ...
        const numbered = results
            ...
            .join('\n\n');
        const prompt = `You are a relevance ranking expert for a Ukrainian-language knowledge base.\n\n` +
        ...
        try {
            ...
            if (!match)
                throw new Error('no rank array in response');
        ...
        for (let i = 0; i < results.length; i += CONCURRENCY) {
            ...
            const batchResults = await Promise.all(batch.map(async (result) => {
                const textSample = result.text.length > 500
                    ...
                    : result.text;
                const prompt = `On a scale of 0 to 10, rate how relevant this text is to the query.\n` +
...
//# sourceMappingURL=reranker.util.js.map