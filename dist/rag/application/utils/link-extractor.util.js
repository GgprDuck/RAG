...
function extractContext(text, matchIndex, maxChars = 300) {
    ...
    while (start > 0) {
        ...
        if (text.slice(prev + 1, start).trim() === '') {
            start = prev + 1;
            break;
        }
        start = prev;
...
function buildKeywords(label, url, context) {
    const kws = new Set();
    const addWithVariants = (w) => {
        kws.add(w);
        for (const v of (ABBR_VARIANTS[w] ?? []))
            kws.add(v);
    };
...
function extractLinksFromMarkdown(content, sourceFile) {
    ...
    const add = (rawUrl, rawLabel, matchIndex) => {
