...
exports.parentChildChunking = parentChildChunking;
const common_1 = require("@nestjs/common");
const logger = new common_1.Logger('Chunking');
function splitIntoSentences(text) {
    const ABBREV_RE = /\b(ст|рр|р|др|проф|доц|вул|пл|просп|обл|кв|год|грн|тис|млн|млрд|ім|пр|т|е|і|іст|буд|корп|оф|мал|табл|рис|п|п-т|с-щ)\./gi;
...
function findBoundary(text, target, windowSize = 200) {
...
function splitMarkdownSections(text) {
    ...
    if (!positions.length)
        return [{ header: null, text, offset: 0 }];
    const sections = [];
    if (positions[0].index > 0) {
        const preamble = text.slice(0, positions[0].index).trim();
        if (preamble)
            sections.push({ header: null, text: preamble, offset: 0 });
    }
...
async function parentChildChunking(text, onChunk, options = {}) {
    const { parentSize = 4000, childSize = 1200, overlap = 150, fileId = 'doc', storeParentText = true, useMarkdownHeaders = true, } = options;
    ...
    if (useMarkdownHeaders) {
        ...
        while (pos < text.length) {
            ...
            if (slice)
                parents.push({ text: slice, offset: pos, header: null });
            pos = boundary;
        }
    }
    ...
    for (let pi = 0; pi < parents.length; pi++) {
        ...
        logger.debug(`${parentId}: ${children.length} children` +
            (header ? ` [${header.slice(0, 50)}]` : ''));
    }
    logger.debug('parentChildChunking: done');
}
function averageEmbeddings(a, b, wa = 1, wb = 1) {
...
//# sourceMappingURL=advanced-chunking.util.js.map