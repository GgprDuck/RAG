...
let TextRagService = TextRagService_1 = class TextRagService {
    constructor(configService, ollama, qdrantService, textRepository, conversationRepository, knowledgeGraph, logger) {
    ...
    cyrillicToLatin(text) {
        return text
            .toLowerCase()
            .split('')
            .map(c => TextRagService_1.CYRILLIC_TO_LATIN[c] ?? c)
            .join('');
    }
    ...
    Відповідай ТІЛЬКИ на основі тексту в <context>.
    ...
    2. Знайди факти, які відповідають на питання
    3. Дай коротку, точну відповідь
    
    ПРАВИЛА:
    - відповідь повинна бути максимально конкретною
    - 1-3 речення
    - якщо інформації недостатньо — напиши "Недостатньо інформації у базі знань"
    ...
    Відповідай ТІЛЬКИ на основі інформації з <context>.
    ЗАБОНЕНО вигадувати факти.
    ...
    2. Визнач ключові теми
    3. Самостійно розділи відповідь на логічні підтеми
    4. Використай всю релевантну інформацію
    
    СТРУКТУРА ВІДПОВІДІ:
    
    - короткий вступ
    - кілька логічних підтем (LLM сам вирішує як їх назвати)
    - під кожною темою дай узагальнену інформацію
    
    ПРАВИЛА:
    
    - використовуй markdown структуру
    - не повторюй факти
    - не додавай інформацію поза context
    - якщо деякі аспекти відсутні — пропусти їх
    ...
    expandToParentContext(results) {
        ...
        for (const { doc, children } of parentGroups.values()) {
            ...
            merged.push({ ...doc, text: combinedText });
        }
...
TextRagService.CYRILLIC_TO_LATIN = {
    'а': 'a', 'б': 'b', 'в': 'v', 'г': 'h', 'ґ': 'g',
...
exports.TextRagService = TextRagService = TextRagService_1 = __decorate([
    ...
    __metadata("design:paramtypes", [config_1.ConfigService,
        ...
        rag_qdrant_service_1.RagQdrantService, Object, Object, Object, Object])
], TextRagService);
