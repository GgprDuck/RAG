...
export interface IConversationSessionRepository {
  ...
  getTurns(sessionId: string, maxTurns?: number): Promise<Array<ConversationTurn & { id: string }>>;
  listSessionHeads(limit?: number): Promise<Array<{ sessionId: string; query: string; timestamp: Date }>>;
