...
export function translateQueryToUkrainian(query: string): string[] {
  ...
  for (const [pattern, ua] of EN_TO_UA_QUERY_PATTERNS) {
    if (pattern.test(query)) results.add(ua);
  }
  return [...results];
}
...
export class QueryTransformer {
  ...
  async transformQuery(query: string, isEntityOverride?: boolean): Promise<TransformedQuery> {
    ...
    if (this.redis) {
      try {
        const cached = await this.redis.get<TransformedQuery>(cacheKey);
    ...
    const result: TransformedQuery = { original: query, expanded, rephrased, keywords, isEntityQuery: entity };
